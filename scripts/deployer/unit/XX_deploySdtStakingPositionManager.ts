...
export async function deploySdtStakingPositionManager(contractsUsers: IContractsUser): Promise<IContractsUser> {
    ...
    await (await sdtStakingPositionManager.transferOwnership(users.treasuryDao)).wait();
    await (await sdtStakingPositionManager.connect(users.treasuryDao).acceptOwnership()).wait();

...
describe("CvgCvxStaking - Claim CvgCVX Rewards", () => {
    ...
    it("Success : claimCvgCvxRewards user2 / token2 for cycle 2 & 3 & 4", async () => {
        ...
        const tx = cvgCvxStaking.connect(users.user2).claimCvgCvxRewards(TOKEN_2, 0, false);
    ...
    it("Fail: set depeg percentage without being owner", async () => {
        await cvgCvxStaking.setDepegPercentage(1001).should.be.revertedWith(OWNABLE_REVERT);
    });

    it("Fail: set depeg percentage with too low percentage", async () => {
        await cvgCvxStaking.connect(treasuryDao).setDepegPercentage(999).should.be.revertedWith("PERCENTAGE_TOO_LOW");
    });

    it("Success: set depeg percentage ", async () => {
        await cvgCvxStaking.connect(treasuryDao).setDepegPercentage(1001);
        expect(await cvgCvxStaking.depegPercentage()).to.be.eq(1001);
    });

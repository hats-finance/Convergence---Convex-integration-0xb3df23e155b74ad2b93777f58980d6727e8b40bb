...
describe("cvgFraxLpStaking - find NFT Staked Amount For CVG Cycle", () => {
    ...
    it("Success : Returns 0 for cycle 0", async () => {
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(0, TOKEN_2, CYCLE_1)).to.be.equal(0);
    });

    it("Success : Returns 0 for current cycle", async () => {
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_1, TOKEN_2, CYCLE_1)).to.be.equal(0);
    });

    it("Success : Returns 0 for unreached cycle", async () => {
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_2, TOKEN_2, CYCLE_1)).to.be.equal(0);
    });
    ...
    it("Success : Returns 0 for a non - existing position", async () => {
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_1, TOKEN_3, CYCLE_2)).to.be.equal(0);
    });
    ...
    it("Success : Returns eligible amount for rewards.", async () => {
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_2, TOKEN_1, CYCLE_5)).to.be.equal(ethers.parseEther("5000"));
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_2, TOKEN_2, CYCLE_5)).to.be.equal(ethers.parseEther("3000"));
    });
    ...
    it("Success : Check eligible amount for all past cycles", async () => {
        ...
        // TOKEN 1
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_1, TOKEN_1, CYCLE_6)).to.be.equal(0);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_2, TOKEN_1, CYCLE_6)).to.be.equal(amount_user1);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_3, TOKEN_1, CYCLE_6)).to.be.equal(amount_user1);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_4, TOKEN_1, CYCLE_6)).to.be.equal(amount_user1);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_5, TOKEN_1, CYCLE_6)).to.be.equal(amount_user1);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_6, TOKEN_1, CYCLE_6)).to.be.equal(0);

        // TOKEN 2
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_1, TOKEN_2, CYCLE_6)).to.be.equal(0);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_2, TOKEN_2, CYCLE_6)).to.be.equal(amount_user2);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_3, TOKEN_2, CYCLE_6)).to.be.equal(amount_user2);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_4, TOKEN_2, CYCLE_6)).to.be.equal(amount_user2);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_5, TOKEN_2, CYCLE_6)).to.be.equal(amount_user2);
        expect(await cvgeUSDFRAXBPStaking.stakedAmountEligibleAtCycle(CYCLE_6, TOKEN_2, CYCLE_6)).to.be.equal(0);
    });
});

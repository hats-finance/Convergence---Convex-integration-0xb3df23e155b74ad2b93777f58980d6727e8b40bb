...
export async function configureAccounts(): Promise<IUsers> {
    ...
    const treasuryTeam = signers[14];
    const treasuryPod = signers[15];
    const treasuryPdd = signers[16];
    const treasuryAirdrop = signers[17];
    const veSdtMultisig = signers[18];
    const treasuryPartners = signers[19];

    const allUsers = [owner, user1, user2, user3, user4, user5, user6, user7, user8, user9, user10, user11, user12, veSdtMultisig];

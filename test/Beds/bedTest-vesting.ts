...
export async function bedTestVestingMintWlTokens(contractsUsers: IContractsUser) {
    ...
    const merkleProof7 = MerkleHelper.getProofMerkle(contracts.presaleVesting.wl.S_wlAddresses, await users.user7.getAddress());
    await presaleContractWl.connect(users.user7).investMint(merkleProof7, ethers.parseEther("200"), true, 2); // 200 to 7 with DAI & SMALL

    const merkleProof4 = MerkleHelper.getProofMerkle(contracts.presaleVesting.wl.M_wlAddresses, await users.user4.getAddress());
    ...
    const merkleProof1 = MerkleHelper.getProofMerkle(contracts.presaleVesting.wl.L_wlAddresses, await users.user1.getAddress());
...
export async function bedTestIboMinting(contractsUsers: IContractsUser) {
    ...
    await iboContract.connect(users.treasuryPod).createBond(REAL_IBO_PARAMETERS.FRAX.BOND_PARAMETERS);
    await iboContract.connect(users.treasuryPod).setStartTimestamp(await time.latest());

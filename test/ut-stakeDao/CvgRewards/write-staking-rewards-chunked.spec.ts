...
import {CYCLE_1} from "../../../resources/constant";
...
describe("CvgRewards / write staking chunk", function () {
    ...
    it("Fails : Changing gauges chunk max size with a random account", async () => {
        await cvgRewards
            .connect(user1)
            .setMaxChunkConfigs({maxChunkCheckpoint: CHUNK_CHECKPOINT, maxChunkDistribute: CHUNK_DISTRIBUTE})
    ...
    it("Success : Changing gauges chunk max size with treasury DAO multisig", async () => {
        await (await cvgRewards.connect(treasuryDao).setMaxChunkConfigs({maxChunkCheckpoint: CHUNK_CHECKPOINT, maxChunkDistribute: CHUNK_DISTRIBUTE})).wait();
    ...
    it("Success : Updating first chunk of gauge Weights on CHECKPOINT state", async () => {
        ...
        let votesAfter = await getGaugeControllerVotes(gaugeController, cvgRewards);
    ...
    it("Success : Cycle 2 - Performing the DISTRIBUTE for cycle 1 first chunk", async () => {
        ...
        for (let index = 0; index < CHUNK_DISTRIBUTE; index++) {
            expect(await gauges[index].stakingCycle()).to.be.eq(CYCLE_2 + 1);

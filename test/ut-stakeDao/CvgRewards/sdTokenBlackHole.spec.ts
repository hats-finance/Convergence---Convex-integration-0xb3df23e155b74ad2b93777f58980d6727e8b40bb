...
describe("sdtBlackHole delegateSdPower", () => {
    ...
    it("Success : delegateSdPower should  emit  ", async () => {
        const id = encodeBytes32String("1");
    ...
    it("Failure : clearDelegate with random user  should be revert the owner", async () => {
        const id = encodeBytes32String("1");
        await sdtBlackHole.connect(user1).clearDelegate(id).should.be.rejectedWith("Ownable: caller is not the owner");
    });
    it("Success : clearDelegate", async () => {
        const id = encodeBytes32String("1");
        await sdtBlackHole.clearDelegate(id);
    });
});

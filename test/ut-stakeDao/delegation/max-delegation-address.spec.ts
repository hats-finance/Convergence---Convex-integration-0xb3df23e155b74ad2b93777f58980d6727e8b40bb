...
describe("LockingPositionManager max delegation per address", () => {
    ...
    it("Set max mg delegatees should revert if not owner of lockingPositionManager", async function () {
        await lockingPositionDelegate.connect(user1).setMaxTokenIdsDelegated(4).should.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Delegates mgCvg of tokenId 1 and 2 to user9 should compute right infos", async () => {
        await lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user9, percentage: 40}]);
        await lockingPositionDelegate.connect(user1).delegateMgCvg(2, [{delegatee: user9, percentage: 20}]);
    ...
    it("Delegate mgCvg tokenId 3 to user9 should revert because max tokenIds delegated is reached", async () => {
        await lockingPositionDelegate
            .connect(user2)
            .delegateMgCvg(3, [{delegatee: user9, percentage: 60}])
            .should.be.revertedWith("TOO_MUCH_MG_TOKEN_ID_DELEGATED");
    });
    ...
    it("Delegate veCVG tokenId 3 to user9 should work because delegated tokens are cleaned", async () => {
        await lockingPositionDelegate.connect(user2).delegateVeCvg(3, user9);
        const tokenIdsVeDelegatedToUser9 = await lockingPositionDelegate.getVeCvgDelegatees(user9);
        tokenIdsVeDelegatedToUser9.length.should.be.equal(1);
    });

    it("Updates delegation when maximum delegation for a token has been reached", async () => {
        await lockingPositionDelegate.connect(treasuryDao).setMaxMgDelegatees(1);
        await lockingPositionDelegate.connect(treasuryDao).setMaxTokenIdsDelegated(1);

        // delegates
        await lockingPositionDelegate.connect(user2).delegateMgCvg(3, [{delegatee: user9, percentage: 10}]);
        // updates delegation when maximum delegation reached (on both side: max token id delegated and maximum delegatees for token id)
        await lockingPositionDelegate.connect(user2).delegateMgCvg(3, [{delegatee: user9, percentage: 25}]);
    });

    it("Fails to delegate mgCVG to another address when maximum has been reached", async () => {
        await lockingPositionDelegate
            .connect(user2)
            .delegateMgCvg(3, [{delegatee: user10, percentage: 10}])
            .should.be.revertedWith("TOO_MUCH_DELEGATEES");
    });

    it("Fails to delegate mgCVG to address that already have maximum token ID delegated to it", async () => {
        await lockingPositionDelegate
            .connect(user1)
            .delegateMgCvg(1, [{delegatee: user9, percentage: 10}])
            .should.be.revertedWith("TOO_MUCH_MG_TOKEN_ID_DELEGATED");
    });

    it("Updates delegation and add new one for a token", async () => {
        await lockingPositionDelegate.connect(treasuryDao).setMaxMgDelegatees(2);

        // update current delegation of user9 and add new delegation to user10
        await lockingPositionDelegate.connect(user2).delegateMgCvg(3, [
            {delegatee: user9, percentage: 60},
            {delegatee: user10, percentage: 20},
        ]);

        const delegationData = await lockingPositionDelegate.getDelegatedMgCvg(3);
        expect(delegationData).to.deep.equal([
            [user9.address, 60n],
            [user10.address, 20n],
        ]);
    });

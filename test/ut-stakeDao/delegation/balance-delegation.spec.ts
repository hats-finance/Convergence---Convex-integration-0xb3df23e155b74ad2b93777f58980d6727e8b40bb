...
describe("LockingPositionManager / mgCVG balance delegation", () => {
    ...
    before(async () => {
        ...
        // MINT POSITION 3
        await (await lockingPositionService.connect(user1).mintPosition(LOCKING_POSITIONS[2].duration, LOCKING_POSITIONS[2].cvgAmount, 60, user1, true)).wait(); // Lock 100 CVG for 43 cycles
    });
    it("Set max mg delegatees should revert if not owner of lockingPositionManager", async function () {
        await lockingPositionDelegate.connect(user1).setMaxMgDelegatees(4).should.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Tries to delegate mgCVG with 150% power to user10", async () => {
        const txFail = lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user10, percentage: 150}]);
        await expect(txFail).to.be.revertedWith("TOO_MUCH_PERCENTAGE");
    });

    it("Tries to delegate mgCVG from non-owner of token", async () => {
        const txFail = lockingPositionDelegate.connect(user2).delegateMgCvg(1, [{delegatee: user10, percentage: 150}]);
        await expect(txFail).to.be.revertedWith("TOKEN_NOT_OWNED");
    });

    it("Tries to remove non-delegated mgCVG user10", async () => {
        const txFail = lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user10, percentage: 0}]);
    ...
    it("Delegates mgCVG with 70% power to user10 and 10% to user2", async () => {
        await expect(lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user10, percentage: 70}]))
            .to.emit(lockingPositionDelegate, "DelegateMetagovernance")
            .withArgs(1, await user10.getAddress(), 70);

        await lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user2, percentage: 10}]);
    ...
    it("Tries to add user3 as delegatee with 50% power for tokenId 1", async () => {
        const txFail = lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user3, percentage: 50}]);
        await expect(txFail).to.be.revertedWith("TOO_MUCH_PERCENTAGE");
    });

    it("Tries to update delegatee user2 with too much percentage for tokenId 1", async () => {
        const txFail = lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user2, percentage: 35}]);
        await expect(txFail).to.be.revertedWith("TOO_MUCH_PERCENTAGE");
    });

    it("Fails to manage tokenIds for user1 with duplicate owneds", async () => {
        let tokenIds = {owneds: [1, 1], mgDelegateds: [], veDelegateds: []};
        const txFail = lockingPositionDelegate.connect(user1).manageOwnedAndDelegated(tokenIds);

        await expect(txFail).to.be.revertedWith("INVALID_OWNED_VALUE");
    });

    it("Fails to manage tokenIds for user1 with not sorted token IDs", async () => {
        let tokenIds = {owneds: [3, 1], mgDelegateds: [], veDelegateds: []};
        const txFail = lockingPositionDelegate.connect(user1).manageOwnedAndDelegated(tokenIds);
    ...
    it("Fails to manage tokenIds for user10 with mg duplicates", async () => {
        let tokenIds = {owneds: [], mgDelegateds: [1, 1], veDelegateds: []};
        const txFail = lockingPositionDelegate.connect(user10).manageOwnedAndDelegated(tokenIds);

        await expect(txFail).to.be.revertedWith("INVALID_MG_VALUE");
    });

    it("Fails to manage tokenIds for user10 with not sorted mg values", async () => {
        // delegate 50% mgCVG to user10
        await lockingPositionDelegate.connect(user1).delegateMgCvg(3, [{delegatee: user10, percentage: 50}]);

        let tokenIds = {owneds: [], mgDelegateds: [3, 1], veDelegateds: []};
        const txFail = lockingPositionDelegate.connect(user10).manageOwnedAndDelegated(tokenIds);
    ...
    it("Removes user10 as delegatee of tokenId 1 and checks data", async () => {
        await lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user10, percentage: 0}]);
    ...
    it("Updates delegatee user2 to 95% and checks data", async () => {
        await lockingPositionDelegate.connect(user1).delegateMgCvg(1, [{delegatee: user2, percentage: 95}]);
    ...
    it("Fails to clean delegatees with direct call to method used during transfer", async () => {
        const txFail = lockingPositionDelegate.connect(user1).cleanDelegateesOnTransfer(1);
        await expect(txFail).to.be.revertedWith("NOT_LOCKING_MANAGER");
    });

    it("Transfers tokenId 1 to user10 and check delegation clean", async () => {
        await lockingPositionManager.connect(user1).transferFrom(user1, user10, 1);

        const mgCvgDelegation = await lockingPositionDelegate.getDelegatedMgCvg(1);
        const veCvgDelegation = await lockingPositionDelegate.delegatedVeCvg(1);
        const ysCvgDelegation = await lockingPositionDelegate.delegatedYsCvg(1);
        expect(mgCvgDelegation).to.be.empty;
        expect(veCvgDelegation).to.be.eq(zeroAddress());
        expect(ysCvgDelegation).to.be.eq(zeroAddress());
    });

    it("Reduces max mgCVG delegatees and fails to add another delegatee", async () => {
        await lockingPositionDelegate.connect(treasuryDao).setMaxMgDelegatees(0);

        const txFail = lockingPositionDelegate.connect(user10).delegateMgCvg(1, [{delegatee: user3, percentage: 5}]);

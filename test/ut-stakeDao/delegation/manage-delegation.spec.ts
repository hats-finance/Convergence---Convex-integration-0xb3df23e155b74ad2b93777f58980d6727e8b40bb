...
describe("LockingPositionManager / veCvg/mgCvg manage delegation", () => {
    ...
    it("Fails to manage tokenIds for user3 with ve duplicates", async () => {
        const tokenIdsUser3 = {owneds: [], mgDelegateds: [], veDelegateds: [1, 1]};
        const txFail = lockingPositionDelegate.connect(user3).manageOwnedAndDelegated(tokenIdsUser3);

        await expect(txFail).to.be.revertedWith("INVALID_VE_VALUE");
    });
    ...
    it("Transfer tokenId to user2", async () => {
        ...
        // re-set delegations because they were cleaned after transfer
        await lockingPositionDelegate.connect(user2).delegateVeCvg(1, user3);
        await lockingPositionDelegate.connect(user2).delegateMgCvg(1, [
            {delegatee: user9, percentage: 40},
            {delegatee: user10, percentage: 20},
        ]);
    });
    ...
    it("Votingpower of user2/user3 should should compute right amount of veCvg if tokenId owned is managed", async () => {
        const veUser2 = await lockingPositionService.veCvgVotingPowerPerAddress(user2);
        const veUser3 = await lockingPositionService.veCvgVotingPowerPerAddress(user3);
        veUser2.should.be.equal("0");

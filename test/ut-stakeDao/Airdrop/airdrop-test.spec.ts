...
describe("Airdrop Tests", () => {
    ...
    before(async () => {
        ...
        merkleRoot = MerkleHelper.getRoot(whitelisted);
    });
    ...
    it("Fail: Claim before active state", async () => {
        const proof = MerkleHelper.getProofMerkle(whitelisted, await user1.getAddress());
    ...
    it("Success: Claim airdrop at cycle 2 with user whitelisted", async () => {
        const proof = MerkleHelper.getProofMerkle(whitelisted, await user2.getAddress());
    ...
    it("Success: Claim airdrop at cycle 12 with user whitelisted", async () => {
        const proof = MerkleHelper.getProofMerkle(whitelisted, await user3.getAddress());
    ...
    it("Fail: Claim airdrop at cycle 12 with user whitelisted without sufficient cvg to claim", async () => {
        const proof = MerkleHelper.getProofMerkle(whitelisted, await user4.getAddress());

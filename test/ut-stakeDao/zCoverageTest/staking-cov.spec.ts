...
import {SdtRewardDistributor, SdtStakingPositionService} from "../../../typechain-types";
...
describe("Coverage Staking ", () => {
    ...
    it("Success: get views", async () => {
        await cvgSdtStakingContract.tokenToId(user1);
        await cvgSdtStakingContract.sdtRewardsByCycle(1, 0);
    });
    ...
    it("initialize stakingPositionManager should revert", async () => {
        await sdtRewardDistributor.initialize(user1).should.be.revertedWith("Initializable: contract is already initialized");
    });

...
describe("BondCalculator / computeCvgExpected", function () {
    ...
    it("should compute CVG expected with sqrt and a 1M target", async () => {
        for (let i = 43200; i < BOND_DURATION; ) {
            cvgExpected = await bondCalculatorContract.computeCvgExpected(i, BOND_DURATION, 0, ethers.parseEther("1000000"));
    ...
    it("should compute CVG expected with ln and a 1M target", async () => {
        for (let i = 43200; i < BOND_DURATION; ) {
            cvgExpected = await bondCalculatorContract.computeCvgExpected(i, BOND_DURATION, 1, ethers.parseEther("1000000"));
    ...
    it("should compute CVG expected with ln and a 50k target", async () => {
        for (let i = 43200; i < BOND_DURATION; ) {
            cvgExpected = await bondCalculatorContract.computeCvgExpected(i, BOND_DURATION, 1, ethers.parseEther("50000"));
    ...
    it("should compute CVG expected with ² and a 1M target", async () => {
        for (let i = 43200; i < BOND_DURATION; ) {
            cvgExpected = await bondCalculatorContract.computeCvgExpected(i, BOND_DURATION, 2, ethers.parseEther("1000000"));

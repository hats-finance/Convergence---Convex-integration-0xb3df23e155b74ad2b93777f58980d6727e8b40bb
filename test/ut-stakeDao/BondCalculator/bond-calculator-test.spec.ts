...
import {deployBondCalculatorFixture} from "../../fixtures/stake-dao";
import {SD_ASSETS_FEE_PERCENTAGE} from "../../../resources/constant";
...
describe("Bond Calculator tests", function () {
    ...
    it("Should compute NumberTokenReal/NumberTokenExpected ratio", async () => {
        ...
        ntrNtcRatio = await bondCalculatorContract.computeNtrDivNtc(1, 86400 * 5, 1, ethers.parseEther("1000"), ethers.parseEther("1"));
        expect(ntrNtcRatio).to.be.eq("0");
    });
    ...
    it("Success : composedFunction != 1", async () => {
        const cvgExpectedA = await bondCalculatorContract.computeCvgExpected(43200, BOND_DURATION, 0, 1000000);
        const cvgExpectedB = await bondCalculatorContract.computeCvgExpected(43200, BOND_DURATION, 0, 1999999);
        expect(cvgExpectedA).to.not.eq(cvgExpectedB);
    });

    it("PoC: composedFunction == 1", async () => {
        const cvgExpectedA = await bondCalculatorContract.computeCvgExpected(43200, BOND_DURATION, 1, ethers.parseEther("1000"));
        const cvgExpectedB = await bondCalculatorContract.computeCvgExpected(43200, BOND_DURATION, 1, ethers.parseEther("1000.999999999999999999"));
        expect(cvgExpectedA).to.not.eq(cvgExpectedB);
    });

    it("Success : With Ln function when cvgExpected returns a negative answer", async () => {
        const cvgExpected = await bondCalculatorContract.computeCvgExpected(2, BOND_DURATION, 1, ethers.parseEther("10000"));
        expect(cvgExpected).to.be.eq(0);
    });

    it("Success : Compute an ROI with ", async () => {
        const cvgExpected = await bondCalculatorContract.computeRoi(2, BOND_DURATION, 1, ethers.parseEther("10000"), 0, 250_000, 5_000, 10_000, 15_000);
        expect(cvgExpected).to.be.eq(15_000);
    });
});

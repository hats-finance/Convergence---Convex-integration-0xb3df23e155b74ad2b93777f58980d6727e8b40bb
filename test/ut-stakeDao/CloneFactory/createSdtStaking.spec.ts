...
import {TOKEN_ADDR_sdCRV_GAUGE} from "../../../resources/tokens/stake-dao";
...
describe("Clone Factory Create sdtStaking", () => {
    ...
    const stakingTokenAddress = TOKEN_ADDR_sdCRV_GAUGE; // sdCRV-gauge
    ...
    it("Initialize base sdAssetStaking contract should be reverted", async () => {
        ...
        await baseSdAssetStakingContract
            .initialize(await controlTowerContract.getAddress(), stakingTokenAddress, "sdCRV-gauge", true, withdrawCallInfo)
    ...
    it("Creates staking contract", async () => {
        ...
        const tx = await cloneFactoryContract.connect(treasuryDao).createSdtStakingAndBuffer(stakingTokenAddress, "sdCRV", false);
        ...
        expect(await stakingContract.symbol()).to.be.eq("sdCRV");

...
import {CYCLE_12, CYCLE_13, CYCLE_24, CYCLE_36, CYCLE_48, CYCLE_5, CYCLE_9, TOKEN_1} from "../../../resources/constant";
...
describe("LockingPositionManager  / increaseLock time", () => {
    ...
    it("Fails : Tries to increase for 0 cycle", async () => {
        const txFail = lockingPositionService.connect(user1).increaseLockTime(TOKEN_1, 0);
    ...
    it("increase staking cycle to 48 and fail to increase amount but success augment time", async () => {
        ...
        let totalSupplyYsCvg = await lockingPositionService.totalSupplyOfYsCvgAt(48);
        expect(totalSupplyYsCvg).to.be.eq("124826388888888888888");
    });
    ...
    it("increase staking cycle to 49", async () => {
        ...
        let totalSupplyYsCvg = await lockingPositionService.totalSupplyOfYsCvgAt(49);
        expect(totalSupplyYsCvg).to.be.eq("124826388888888888888");
    });
    ...
    it("increase staking cycle to 61", async () => {
        ...
        let totalSupplyYsCvg = await lockingPositionService.totalSupplyOfYsCvgAt(73);
        expect(totalSupplyYsCvg).to.be.eq(0);
    });

    it("fails claiming because no lock are active on Convergence", async () => {
        await ysdistributor.connect(user1).claimRewards(TOKEN_1, 6, user1).should.be.rejectedWith("NO_YS_BALANCE_ON_THIS_TDE");
    });
});

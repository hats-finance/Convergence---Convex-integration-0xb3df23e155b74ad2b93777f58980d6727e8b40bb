...
import {CvgSdtBuffer, IFeeDistributor, SdtRewardDistributor} from "../../../../../typechain-types";
...
import {CLAIMER_REWARDS_PERCENTAGE, CYCLE_2, CYCLE_3, CYCLE_4, TOKEN_4, TOKEN_5} from "../../../../../resources/constant";
...
describe("cvgSdtStaking - Claim Cvg Rewards", () => {
    ...
    before(async () => {
        ...
        sdtRewardDistributor = contracts.stakeDao.sdtRewardDistributor;
        cvg = tokens.cvg;
    ...
    it("Fail: claimCvgSdtMultiple with other caller than sdtRewardDistributor contract", async () => {
        await cvgSdtStakingContract.claimCvgSdtMultiple(0, user1).should.be.revertedWith("NOT_SDT_REWARD_RECEIVER");
    });
    it("Fail: setPoolCvgSdtAndApprove with random user", async () => {
        await sdtRewardDistributor.setPoolCvgSdtAndApprove(ethers.ZeroAddress, 0).should.be.revertedWith("Ownable: caller is not the owner");
    });
    it("Success: setPoolCvgSdtAndApprove twice with 2 differents address", async () => {
        await sdtRewardDistributor.connect(treasuryDao).setPoolCvgSdtAndApprove(user1, ethers.parseEther("1"));
        await sdtRewardDistributor.connect(treasuryDao).setPoolCvgSdtAndApprove(user2, ethers.parseEther("1"));
        expect(await sdt.allowance(sdtRewardDistributor, user1)).to.be.equal("0");
        expect(await sdt.allowance(sdtRewardDistributor, user2)).to.be.equal(ethers.parseEther("1"));
    });
    it("Fail: claimCvgSdtSimple with other caller than staking contract should revert", async () => {
        await sdtRewardDistributor
            .claimCvgSdtSimple(ethers.ZeroAddress, 0, [{token: ethers.ZeroAddress, amount: 0}], 0, false)
    ...
    it("Success : Process SDT rewards for cycle 2.", async () => {
        ...
        const cvgSdtForProcessor = (balanceCvgSdtBuffer * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;
        const cvgSdtDistributed = balanceCvgSdtBuffer - cvgSdtForProcessor;

        const balSdtBuffer = await sdt.balanceOf(cvgSdtBuffer);
        const sdtForProcessor = (balSdtBuffer * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;
        ...
        await expect(txProcess).to.changeTokenBalances(
            cvgSdt,
            [cvgSdtBuffer, sdtRewardDistributor, veSdtMultisig],
            [-balanceCvgSdtBuffer, cvgSdtDistributed, cvgSdtForProcessor]
        );
        await expect(txProcess).to.changeTokenBalances(
            sdt,
            [cvgSdtBuffer, sdtRewardDistributor, veSdtMultisig],
            [-balSdtBuffer, sdtDistributed, sdtForProcessor]
        );
    });
    ...
    it("claimRewards cycle 2 with wrong owner should revert", async () => {
        ...
        await cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_4, 0, false).should.be.revertedWith("TOKEN_NOT_OWNED");
    });
    ...
    it("claimRewards cycle 2 for user 1 should compute right infos", async () => {
        ...
        const claimSdtTx = cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false);

        await expect(claimSdtTx).to.changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-sdtAmountExpected, sdtAmountExpected]);
        await expect(claimSdtTx).to.changeTokenBalances(sdFRAX3CRV, [sdtRewardDistributor, user1], [-sdFrax3CrvAmountExpected, sdFrax3CrvAmountExpected]);
    ...
    it("claimRewards cycle 2 for user 2 should compute right infos", async () => {
        ...
        const claimSdtTx = cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_5, 0, false);
        await expect(claimSdtTx).to.changeTokenBalances(sdt, [sdtRewardDistributor, user2], [-sdtAmountExpected, sdtAmountExpected]);
        await expect(claimSdtTx).to.changeTokenBalances(sdFRAX3CRV, [sdtRewardDistributor, user2], [-sdFrax3CrvAmountExpected, sdFrax3CrvAmountExpected]);
    ...
    it("Re-claim cycle 2 should revert", async () => {
        ...
        await cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(4, 0, false).should.be.revertedWith("ALL_SDT_CLAIMED_FOR_NOW");
        await cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(5, 0, false).should.be.revertedWith("ALL_SDT_CLAIMED_FOR_NOW");
    });
    ...
    it("process Sdt rewards for cycle 3 should compute right infos", async () => {
        ...
        const cvgSdtForProcessor = (amountCvgSdt * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;
        const cvgSdtDistributed = amountCvgSdt - cvgSdtForProcessor;

        const sdtForProcessor = (amountSdt * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;
    ...
    it("claimRewards cycle 3 for user 1 should compute right infos", async () => {
        ...
        const claimSdtTx = cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(4, 0, false);
        await expect(claimSdtTx).to.changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-sdtAmountExpected, sdtAmountExpected]);
        await expect(claimSdtTx).to.changeTokenBalances(sdFRAX3CRV, [sdtRewardDistributor, user1], [-sdFrax3CrvAmountExpected, sdFrax3CrvAmountExpected]);
    ...
    it("Fails : Re-claim cycle 3 should revert", async () => {
        ...
        await cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false).should.be.revertedWith("ALL_SDT_CLAIMED_FOR_NOW");
        await cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_5, 0, false).should.be.revertedWith("ALL_SDT_CLAIMED_FOR_NOW");
    });
    ...
    it("Success : Processing Sdt rewards for cycle 4 should compute right infos", async () => {
        const amountSdt = (ethers.parseEther("800") * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;
        const amountCvgSdt = (ethers.parseEther("300") * CLAIMER_REWARDS_PERCENTAGE) / 100_000n;

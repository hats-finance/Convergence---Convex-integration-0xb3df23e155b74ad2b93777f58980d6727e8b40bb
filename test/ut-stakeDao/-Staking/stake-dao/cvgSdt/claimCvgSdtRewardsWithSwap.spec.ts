...
import {ICrvPoolPlain, ERC20, Cvg, CvgSDT, LockingPositionService, CvgSdtBuffer, SdtStakingPositionService} from "../../../../../typechain-types";
...
describe("cvgSdtStaking - Claim CvgSdt Rewards With Swap Or Mint", () => {
    ...
    it("Success : ClaimCvgSdtRewards for cycle 2 with convert and mint", async () => {
        ...
        const tx = cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, true);
        ...
        expect(await cvgSdtStakingContract.nextClaims(TOKEN_4)).to.be.deep.equal([CYCLE_3, CYCLE_3]);
    });

    it("Fails : ClaimCvgSdtRewards for cycle 2 with convert only with min SDT amount too high", async () => {
        ...
        const tx = cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_5, sdt_dy + 1n, true);
    ...
    it("Success : ClaimCvgSdtRewards for cycle 2 with convert only", async () => {
        ...
        const slippage = 5n;
        const minCvgSdtAmountOut = sdt_dy - (sdt_dy * slippage) / 1000n; // 0.5%

        const tx = cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_5, minCvgSdtAmountOut, true);
        ...
        expect(await cvgSdtStakingContract.nextClaims(TOKEN_5)).to.be.deep.equal([CYCLE_3, CYCLE_3]);
    });
});

...
import {IFeeDistributor, SdtRewardDistributor} from "../../../../../typechain-types";
...
describe("CvgSdtStaking - Claim CvgSdt Rewards ", () => {
    ...
    it("Fail: setProcessorRewardsPercentage with random user", async () => {
        await cvgSdtBuffer.setProcessorRewardsPercentage(3500).should.be.revertedWith("Ownable: caller is not the owner");
    });
    it("Fail: setProcessorRewardsPercentage with percentage too high", async () => {
        await cvgSdtBuffer.connect(treasuryDao).setProcessorRewardsPercentage(3500).should.be.revertedWith("PERCENTAGE_TOO_HIGH");
    });
    it("Success: setProcessorRewardsPercentage", async () => {
        await cvgSdtBuffer.connect(treasuryDao).setProcessorRewardsPercentage(1000);
    });
    ...
    it("Success : Verifying tokenInfoByCycle initial state", async () => {
        ...
        expect(await cvgSdtStakingContract.cycleInfo(CYCLE_1)).to.be.deep.eq([0, 0, true]);
        expect(await cvgSdtStakingContract.cycleInfo(CYCLE_2)).to.be.deep.eq([0, depositedAmountToken4 + depositedAmountToken5, false]);
    });
    ...
    it("Success : Withdrawing user2 at cycle 2", async () => {
        ...
        expect(await cvgSdtStakingContract.cycleInfo(CYCLE_2)).to.be.deep.eq([0, depositedAmountToken4 + depositedAmountToken5 - withdrawAmount, false]);
        expect(await cvgSdtStakingContract.cycleInfo(CYCLE_3)).to.be.deep.eq([0, depositedAmountToken4 + depositedAmountToken5 - withdrawAmount, false]);
    });
    ...
    it("Success : Claim Token 4 CvgSdt Rewards for cycle 2 & 3 & 4. CVG Are already claimed here.", async () => {
        ...
        const cycle3RewardsExpected = await getExpectedCvgSdtRewards(cvgSdtStakingContract, TOKEN_4, CYCLE_3);
        const cycle4RewardsExpected = await getExpectedCvgSdtRewards(cvgSdtStakingContract, TOKEN_4, CYCLE_4);
        const tx = cvgSdtStakingContract.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false);
        ...
        await expect(tx).to.changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-totalSdt, totalSdt]);
        await expect(tx).to.changeTokenBalances(sdFRAX3CRV, [sdtRewardDistributor, user1], [-totalSdFrax3CRV, totalSdFrax3CRV]);
    ...
    it("Success : claimCvgSdtRewards user2 / token2 for cycle 2 & 3 & 4", async () => {
        ...
        const tx = cvgSdtStakingContract.connect(user2).claimCvgSdtRewards(TOKEN_5, 0, false);
        const totalCvg = cycle2RewardsExpected[0] + cycle3RewardsExpected[0] + cycle4RewardsExpected[0];
        const totalSdt = cycle2RewardsExpected[1] + cycle3RewardsExpected[1] + cycle4RewardsExpected[1];
        const totalSdFrax3CRV = cycle2RewardsExpected[2] + cycle3RewardsExpected[2] + cycle4RewardsExpected[2];
        ...
        await expect(tx).to.changeTokenBalances(sdt, [sdtRewardDistributor, user2], [-totalSdt, totalSdt]);
        await expect(tx).to.changeTokenBalances(sdFRAX3CRV, [sdtRewardDistributor, user2], [-totalSdFrax3CRV, totalSdFrax3CRV]);

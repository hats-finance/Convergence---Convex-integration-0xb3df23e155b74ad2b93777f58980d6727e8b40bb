...
import {
    CLAIMER_REWARDS_PERCENTAGE,
    CYCLE_2,
    CYCLE_3,
    CYCLE_4,
    CYCLE_5,
    DENOMINATOR,
    SD_ASSETS_FEE_PERCENTAGE,
    TOKEN_4,
    TOKEN_5,
...
describe("sdAssetStaking - Sdt Rewards Different Order", () => {
    ...
    let gaugeController: GaugeController,
        ...
        sdtRewardsReceiver: SdtRewardDistributor;
    let cvg: Cvg;
    ...
    before(async () => {
        ...
        sdtRewardsReceiver = contractsUsers.contracts.stakeDao.sdtRewardDistributor;
        cvg = tokens.cvg;
        sdt = tokens.sdt;
    ...
    it("Success : Adding a bribe reward on the sdtBlackHole linked to the buffer", async () => {
        await sdtBlackHole.setBribeTokens(
            [
                {token: sdCrv, fee: SD_ASSETS_FEE_PERCENTAGE},
                {token: bal, fee: SD_ASSETS_FEE_PERCENTAGE},
                {token: usdc, fee: SD_ASSETS_FEE_PERCENTAGE},
            ],
            sdFXSStakingBuffer
        );
        const bribesTokens = await sdtBlackHole.getBribeTokensForBuffer(sdFXSStakingBuffer);
        expect(bribesTokens[0]).to.be.deep.equal([await sdCrv.getAddress(), SD_ASSETS_FEE_PERCENTAGE]);
        expect(bribesTokens[1]).to.be.deep.equal([await bal.getAddress(), SD_ASSETS_FEE_PERCENTAGE]);
        expect(bribesTokens[2]).to.be.deep.equal([await usdc.getAddress(), SD_ASSETS_FEE_PERCENTAGE]);
    });
    ...
    it("Success : Processing SDT rewards for cycle 2", async () => {
        ...
        sdCrvBribeCycle2 -= (sdCrvBribeCycle2 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
        balBribeCycle2 -= (balBribeCycle2 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
        usdcBribeCycle2 -= (usdcBribeCycle2 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
    ...
    it("Success : Claiming Cvg & Sdt rewards cycle 2 for user 1 ", async () => {
        ...
        const claimCvgSdtTx = await sdFXSGaugeStaking.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false);
        ...
        expect(await sdFXSGaugeStaking.nextClaims(TOKEN_4)).to.be.deep.equal([CYCLE_3, CYCLE_3]);
    });
    it("Success : Claiming Cvg & Sdt rewards cycle 2 for user 2 ", async () => {
        ...
        const claimCvgSdtTx = await sdFXSGaugeStaking.connect(user2).claimCvgSdtRewards(TOKEN_5, 0, false);
        ...
        expect(await sdFXSGaugeStaking.nextClaims(TOKEN_5)).to.be.deep.equal([CYCLE_3, CYCLE_3]);
    });
    it("Success : Adding a bribe reward on the sdtBlackHole linked to the buffer", async () => {
        await sdtBlackHole.setBribeTokens(
            [
                {token: bal, fee: SD_ASSETS_FEE_PERCENTAGE},
                {token: frax, fee: SD_ASSETS_FEE_PERCENTAGE},
                {token: usdc, fee: SD_ASSETS_FEE_PERCENTAGE},
            ],
            sdFXSStakingBuffer
        );
    ...
    it("Success : Processing SDT rewards for cycle 3", async () => {
        const processSdtRewardsTx = await sdFXSGaugeStaking.processSdtRewards();
        //bribes rewards balance
        balBribeCycle3 -= (balBribeCycle3 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
        fraxBribeCycle3 -= (fraxBribeCycle3 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
        usdcBribeCycle3 -= (usdcBribeCycle3 * (CLAIMER_REWARDS_PERCENTAGE + SD_ASSETS_FEE_PERCENTAGE)) / DENOMINATOR;
    ...
    it("Success : Claiming Cvg & Sdt rewards cycle 3 for user 1 ", async () => {
        ...
        expect(await sdFXSGaugeStaking.nextClaims(TOKEN_4)).to.be.deep.equal([CYCLE_4, CYCLE_4]);
    });
    ...
    it("Success : Claiming Cvg & Sdt rewards cycle 3 for user 2 ", async () => {
        ...
        await expect(claimCvgSdtTx).to.changeTokenBalances(cvg, [user2], [cvgAmountExpected]);
        expect(await sdFXSGaugeStaking.nextClaims(TOKEN_5)).to.be.deep.equal([CYCLE_4, CYCLE_4]);
    });

...
describe("SdtStakingService - Withdraw", () => {
    ...
    it("Success : Removes all staked assets in N+2", async () => {
        ...
        expect(stakingInfo).to.be.deep.eq([TOKEN_2, "sdANGLE", 0, 0, cvgDistributionCycle2, []]);
    ...
    it("Success : Deposits in cycle 3 for user 2 after emptying the position", async () => {
        ...
        expect(stakingInfo).to.be.deep.eq([2n, "sdANGLE", stakingAmountCycle3, 0, cvgDistributionCycle2, []]);
    ...
    it("Success : Claim all ", async () => {
        ...
        expect(await sdCRVStaking.nextClaims(TOKEN_1)).to.be.deep.equal([CYCLE_6, 0]);
        expect(await sdANGLEStaking.nextClaims(TOKEN_2)).to.be.deep.equal([CYCLE_6, 0]);
        expect(await sdCRVStaking.nextClaims(TOKEN_4)).to.be.deep.equal([CYCLE_6, 0]);
        expect(await sdCRVStaking.nextClaims(TOKEN_4)).to.be.deep.equal([CYCLE_6, 0]);
    });
});

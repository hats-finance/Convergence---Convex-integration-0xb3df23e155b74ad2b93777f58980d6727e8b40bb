...
describe("SdtStaking - Claim  CvgSdt after a cycle triggered for CVG without processing SDT.", () => {
    ...
    let sdCRVStaking: SdtStakingPositionService, sdCrvGauge: ISdAssetGauge, sdCRVBuffer: SdtBuffer, sdtRewardDistributor: SdtRewardDistributor;
    ...
    it("Succees : claimCvgSdt for cycle 2", async () => {
        ...
        const claimTx = sdCRVStaking.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false);

        await expect(claimTx).changeTokenBalance(cvg, user1, cycle2RewardsExpected[0]);
        await expect(claimTx).changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-cycle2RewardsExpected[1], cycle2RewardsExpected[1]]);
        await expect(claimTx).changeTokenBalances(_3crv, [sdtRewardDistributor, user1], [-cycle2RewardsExpected[2], cycle2RewardsExpected[2]]);
        await expect(claimTx).changeTokenBalances(crv, [sdtRewardDistributor, user1], [-cycle2RewardsExpected[3], cycle2RewardsExpected[3]]);
        expect(await sdCRVStaking.nextClaims(TOKEN_4)).to.be.deep.eq([CYCLE_3, CYCLE_3]);
    });
    ...
    it("Success : claimCvgSdt for cycle 3 on Sdt rewards & Cycle 3 & 4 on Cvg because processSdtRewards of cycle 4 is not done yet.", async () => {
        ...
        await expect(claimTx).changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-cycle3RewardsExpected[1], cycle3RewardsExpected[1]]);
        await expect(claimTx).changeTokenBalances(_3crv, [sdtRewardDistributor, user1], [-cycle3RewardsExpected[2], cycle3RewardsExpected[2]]);
        await expect(claimTx).changeTokenBalances(crv, [sdtRewardDistributor, user1], [-cycle3RewardsExpected[3], cycle3RewardsExpected[3]]);
        expect(await sdCRVStaking.nextClaims(TOKEN_4)).to.be.deep.eq([CYCLE_5, CYCLE_4]);
    });
    ...
    it("Success : claimCvgSdt for cycle 4 ONLY for Sdt rewards, Cycle 4 for CVG already claimed before", async () => {
        ...
        await expect(claimTx).changeTokenBalances(sdt, [sdtRewardDistributor, user1], [-cycle4RewardsExpected[1], cycle4RewardsExpected[1]]);
        await expect(claimTx).changeTokenBalances(_3crv, [sdtRewardDistributor, user1], [-cycle4RewardsExpected[2], cycle4RewardsExpected[2]]);
        await expect(claimTx).changeTokenBalances(crv, [sdtRewardDistributor, user1], [-cycle4RewardsExpected[3], cycle4RewardsExpected[3]]);
        expect(await sdCRVStaking.nextClaims(TOKEN_4)).to.be.deep.eq([CYCLE_5, CYCLE_5]);
    });

    it("Fails : claimCvgSdt for cycle 4 only", async () => {
        await sdCRVStaking.connect(user1).claimCvgSdtRewards(TOKEN_4, 0, false).should.be.revertedWith("ALL_SDT_CLAIMED_FOR_NOW");
    });

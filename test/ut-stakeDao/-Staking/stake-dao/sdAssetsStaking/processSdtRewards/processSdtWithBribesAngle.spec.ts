...
import {CLAIMER_REWARDS_PERCENTAGE, CYCLE_2, DENOMINATOR, SD_ASSETS_FEE_PERCENTAGE, TOKEN_4} from "../../../../../../resources/constant";
...
describe("sdAssetStaking - SD_ANGLE - process SDT rewards with bribes", () => {
    ...
    let owner: Signer, user1: Signer, user2: Signer, treasuryDao: Signer, treasuryPod: Signer;
    ...
    it("Success : claimed amounts are the full amount", async () => {
        await sdANGLEStaking.connect(user2).claimCvgSdtRewards(TOKEN_4, 0, false);
    });

    it("Success : Add a bribe reward on the sdtBlackHole linked to the buffer", async () => {
        await sdtBlackHole.setBribeTokens([{token: contractsUsers.contracts.tokensStakeDao.sdAngle, fee: SD_ASSETS_FEE_PERCENTAGE}], sdAngleStakingBuffer);
    });
    ...
    it("Verify : Amounts in SDT sent to the Staking contract & to the FeeCollector", async () => {
        ...
        await expect(processSdtRewardsTx).to.changeTokenBalances(
            sdt,
            [sdAngleStakingBuffer, sdtRewardDistributor, sdtFeeCollector, owner],
    ...
    it("Verify : Amounts of gauge rewards SAN EUR USD ", async () => {
        ...
        await expect(processSdtRewardsTx).to.changeTokenBalances(
            sanUsdEur,
            [sdAngleStakingBuffer, sdtRewardDistributor, owner],
    ...
    it("Verify : sdANGLE bribes are claimed", async () => {
        const feeSdAssets = (sdAngleBribe * SD_ASSETS_FEE_PERCENTAGE) / DENOMINATOR;
        const claimerRewards = (sdAngleBribe * CLAIMER_REWARDS_PERCENTAGE) / DENOMINATOR;

        await expect(processSdtRewardsTx).to.changeTokenBalances(
            sdAngle,
            [sdtBlackHole, sdtRewardDistributor, owner, treasuryPod],
            [-sdAngleBribe, sdAngleBribe - claimerRewards - feeSdAssets, claimerRewards, feeSdAssets]
        );

        const rewardWritten = rewardsWrittenCycle3[4];

        expect(rewardWritten.token).to.be.eq(await sdAngle.getAddress());
        expect(rewardWritten.amount).to.be.eq(sdAngleBribe - claimerRewards - feeSdAssets);
    });
});

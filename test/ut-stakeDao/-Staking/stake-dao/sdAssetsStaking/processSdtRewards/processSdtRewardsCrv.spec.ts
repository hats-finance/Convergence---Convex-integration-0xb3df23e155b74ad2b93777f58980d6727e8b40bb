...
import {CvgControlTower, ERC20, ISdAssetGauge, SdtBlackHole, SdtBuffer, SdtFeeCollector, SdtRewardDistributor} from "../../../../../../typechain-types";
...
describe("sdAssetStaking - SD_CRV - process SDT rewards without bribes", () => {
    let contractsUsers: IContractsUser, users: IUsers;
    let owner: Signer, user1: Signer;
    let sdtFeeCollector: SdtFeeCollector, sdtBlackHole: SdtBlackHole, sdCrvStakingBuffer: SdtBuffer, sdtRewardDistributor: SdtRewardDistributor;
    ...
    it("Success : claimed amounts are the full amount", async () => {
        const claimSdtTx = sdCRVStaking.connect(user1).claimCvgSdtRewards(TOKEN_1, 0, false);
    ...
    it("Verify : Amounts in SDT sent to the SdtRewardDistributor & to the FeeCollector", async () => {
        ...
        await expect(processSdtRewardsTx).to.changeTokenBalances(
            ...
            [sdCrvGauge, sdtRewardDistributor, sdtFeeCollector, owner],
    ...
    it("Verify : Amounts in other tokens are sent to SdtRewardDistributor contract", async () => {
        ...
        for (let index = 1; index < allClaimableRewards.length; index++) {
            ...
            await expect(processSdtRewardsTx).to.changeTokenBalances(
                ...
                [sdCrvGauge, sdtRewardDistributor, owner],
    ...
    it("Verify : Amounts in SDT sent to the Staking contract & to the FeeCollector", async () => {
        ...
        await expect(processSdtRewardsTx).to.changeTokenBalances(
            sdt,
            [sdCrvStakingBuffer, sdtRewardDistributor, sdtFeeCollector, owner],
    ...
    it("Verify : Amounts in other tokens are sent to SdAssetStaking contract", async () => {
        ...
        for (let index = 1; index < allClaimableRewards.length; index++) {
            ...
            await expect(processSdtRewardsTx).to.changeTokenBalances(
                claimableRewards.token,
                [sdCrvStakingBuffer, sdtRewardDistributor, owner],

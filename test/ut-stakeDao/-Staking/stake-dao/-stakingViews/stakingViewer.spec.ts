...
import {
    ...
    TOKEN_ADDR_triUSDC_GAUGE,
    TOKEN_ADDR_triUSDT_GAUGE,
    TOKEN_ADDR_triUSDT2_GAUGE,
...
describe("SdtStakingViewer - Test view function SDT", () => {
    ...
    it("Success : Creating several LP assets", async () => {
        await cloneFactory.connect(users.treasuryDao).createSdtStakingAndBuffer(TOKEN_ADDR_triUSDC_GAUGE, "triUSDC", true);
        await cloneFactory.connect(users.treasuryDao).createSdtStakingAndBuffer(TOKEN_ADDR_triUSDT_GAUGE, "triUSDT", true);
        await cloneFactory.connect(users.treasuryDao).createSdtStakingAndBuffer(TOKEN_ADDR_triUSDT2_GAUGE, "triUSDT2", true);
        await cloneFactory.connect(users.treasuryDao).createSdtStakingAndBuffer(TOKEN_ADDR_frxETH_ETH_GAUGE, "frxEth-Eth", true);
    });
    ...
    it("Checks claimable rewards amount for each sdAsset-gauge", async () => {
        ...
        const sdCrvProcessableRewards = stakingProcessableRewards.find((reward) => reward.sdAssetGauge === TOKEN_ADDR_sdCRV_GAUGE);
        ...
        const expectedClaimableRewards = await Promise.all(
            gaugeRewards.map(async (rewards) => ({

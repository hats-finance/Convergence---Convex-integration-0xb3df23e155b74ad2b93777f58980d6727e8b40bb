...
describe("SdtFeeCollector - Config", () => {
    ...
    let user1: Signer, user2: Signer, treasuryPod: Signer, treasuryDao: Signer;
    ...
    it("Success : Verify initial parameters", async () => {
        expect(await sdtFeeCollector.rootFees()).to.be.eq(15_000);
        ...
        expect(fee0.feePercentage).to.be.eq(67_000n);

        expect(fee1.receiver).to.be.eq(await treasuryPod.getAddress());
        expect(fee1.feePercentage).to.be.eq(33_000n);
    });

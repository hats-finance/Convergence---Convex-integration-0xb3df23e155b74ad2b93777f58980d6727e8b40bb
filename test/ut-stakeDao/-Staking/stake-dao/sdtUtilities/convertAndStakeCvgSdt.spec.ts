...
describe("SdtUtilities - Convert & stake CvgSdt", () => {
    ...
    before(async () => {
        ...
        const setStablePools: SdtUtilities.SetStablePoolStruct[] = [{liquidLocker: cvgSdt, lp: stablePoolCvgSdt}];
    ...
    it("Success : Create position X/X", async () => {
        const tx = sdtUtilities.connect(user1).convertAndStakeCvgSdt(0, ONE_ETHER, ONE_HUNDRED_ETHER, ONE_HUNDRED_ETHER);
    ...
    it("Success : Create position 0/X", async () => {
        const tx = await sdtUtilities.connect(user1).convertAndStakeCvgSdt(0, 0, TEN_ETHER, TEN_ETHER);
    ...
    it("Success : Create position X/0", async () => {
        const tx = await sdtUtilities.connect(user1).convertAndStakeCvgSdt(0, THREE_ETHER, THREE_ETHER, 0);
    ...
    it("Success : Reverse the Peg by selling some CvgSdt", async () => {
        ...
        await cvgSdt.connect(user1).mint(user1, ethers.parseEther("25000"));

        await lpStable.connect(user1).exchange(1, 0, ethers.parseEther("25000"), 0, user1);
    });
    ...
    it("Success : Swap & stake CvgSdt through swapping in LP", async () => {
        ...
        const tx = sdtUtilities.connect(user1).convertAndStakeCvgSdt(4, TEN_ETHER, TEN_ETHER, TEN_ETHER);

        // Here veSdtMultisig doesn't receive any Sdt because the swap is performed through the LP
        await expect(tx).to.changeTokenBalances(sdt, [user1, veSdtMultisig], [-TEN_ETHER, 0]);
        balanceDelta = (await cvgSdt.balanceOf(cvgSdtStaking)) - balanceDelta;
        expect(balanceDelta).to.be.gte((TEN_ETHER * 1025n) / 1000n);

        await expect(tx).to.changeTokenBalances(cvgSdt, [user1, cvgSdtStaking], [-TEN_ETHER, balanceDelta]);

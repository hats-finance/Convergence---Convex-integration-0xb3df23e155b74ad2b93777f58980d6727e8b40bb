...
describe("SdtUtilities - Convert & stake CRV", () => {
    ...
    before(async () => {
        ...
        const setStablePools: SdtUtilities.SetStablePoolStruct[] = [{liquidLocker: sdCrv, lp: CRV_DUO_SDCRV_CRV}];
        await sdtUtilities.setStablePools(setStablePools);
    ...
    it("Success : Create position X/X/X", async () => {
        ...
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, ONE_ETHER, THREE_ETHER, TWO_ETHER, THREE_ETHER, false);
    ...
    it("Success : Create position 0/0/X", async () => {
        const sdCrvGaugeBalanceOfBlackHoleBefore = await sdCrvGauge.balanceOf(sdtBlackHole);
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, 0, ONE_HUNDRED_ETHER, 0, ONE_HUNDRED_ETHER, false);
    ...
    it("Success : Create position X/0/X", async () => {
        const sdCrvGaugeBalanceOfBlackHoleBefore = await sdCrvGauge.balanceOf(sdtBlackHole);
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, ONE_HUNDRED_ETHER, TEN_ETHER, 0, TEN_ETHER, false);
    ...
    it("Success : Create position 0/X/0", async () => {
        const sdCrvGaugeBalanceOfBlackHoleBefore = await sdCrvGauge.balanceOf(sdtBlackHole);
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, 0, 0, TWO_ETHER, 0, false);
    ...
    it("Success : Create position 0/X/X", async () => {
        const sdCrvGaugeBalanceOfBlackHoleBefore = await sdCrvGauge.balanceOf(sdtBlackHole);
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, 0, ONE_ETHER, TEN_ETHER, ONE_ETHER, false);
    ...
    it("Success : Create position X/0/0", async () => {
        const sdCrvGaugeBalanceOfBlackHoleBefore = await sdCrvGauge.balanceOf(sdtBlackHole);
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdCRVStaking, ONE_HUNDRED_ETHER, 0, 0, 0, false);
        await expect(tx).to.changeTokenBalance(crv, user1, 0);
        await expect(tx).to.changeTokenBalance(sdCrv, user1, 0);
        const sdCrvGaugeDelta = (await sdCrvGauge.balanceOf(sdtBlackHole)) - sdCrvGaugeBalanceOfBlackHoleBefore;
        await expect(tx).to.changeTokenBalances(sdCrvGauge, [user1, sdtBlackHole], [-ONE_HUNDRED_ETHER, sdCrvGaugeDelta]);
    ...
    it("Success : Create position 0/0/X without stable pool setted", async () => {
        ...
        const tx = sdtUtilities.connect(user1).convertAndStakeSdAsset(0, sdBALStaking, 0, ONE_HUNDRED_ETHER, 0, ONE_HUNDRED_ETHER, false);

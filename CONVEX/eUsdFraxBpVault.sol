...
contract StakingProxyConvex is StakingProxyBase, ReentrancyGuard {
    ...
    function initialize(
        ...
        (address _lptoken, address _token, , , , ) = ICurveConvex(convexCurveBooster).poolInfo(
            IConvexWrapperV2(_stakingToken).convexPoolId()
        );
    ...
    function stakeLockedCurveLp(
        uint256 _liquidity,
        uint256 _secs
    ) external onlyOwner nonReentrant returns (bytes32 kek_id) {
        if (_liquidity > 0) {

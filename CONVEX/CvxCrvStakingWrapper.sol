...
contract CvxCrvStakingWrapper is ERC20, ReentrancyGuard {
    ...
    function addRewards() internal {
        if (rewards.length == 0) {
            rewards.push(RewardType({reward_token: crv, reward_integral: 0, reward_remaining: 0, reward_group: 0}));
            rewards.push(RewardType({reward_token: cvx, reward_integral: 0, reward_remaining: 0, reward_group: 0}));
            rewards.push(
                RewardType({reward_token: threeCrv, reward_integral: 0, reward_remaining: 0, reward_group: 1})
            );
    ...
    function addTokenReward(address _token, uint256 _rewardGroup) public onlyOwner nonReentrant {
        require(
            _token != address(0) && _token != cvxCrvStaking && _token != address(this) && _token != cvxCrv,
            "invalid address"
        );
            ...
            if (_isClaim || userI < reward.reward_integral) {
                if (_isClaim) {
                    uint256 receiveable = reward.claimable_reward[_accounts[u]].add(
                        userb.mul(uint256(reward.reward_integral).sub(userI)).div(1e20)
                    );
                    if (receiveable > 0) {

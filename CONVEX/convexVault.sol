...
contract Booster {
    ...
    constructor(address _proxy, address _poolReg, address _feeReg) {
        ...
        //temple
        proxyOwners[address(0xC0223fB0562555Bec938de5363D63EDd65102283)] = address(
            0x4A136F836961860E599d9BF6e03BBb4BcD0E39dd
        );
    }
    ...
    function voteGaugeWeight(
        ...
        for (uint256 i = 0; i < _gauge.length; ) {
            bytes memory data = abi.encodeWithSelector(
                bytes4(keccak256("vote_for_gauge_weights(address,uint256)")),
                _gauge[i],
                _weight[i]
            );
    ...
    function castVote(address _votingContract, uint256 _proposalId, bool _support) external {
        ...
        bytes memory data = abi.encodeWithSelector(
            bytes4(keccak256("castVote(uint256,uint8)")),
            _proposalId,
            _support ? uint8(1) : uint8(0)
        );

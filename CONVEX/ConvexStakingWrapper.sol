...
contract ConvexStakingWrapper is ERC20, ReentrancyGuard {
    ...
    function addRewards() public {
        ...
        if (rewards.length == 0) {
            ...
            rewards.push(
                RewardType({reward_token: cvx, reward_pool: address(0), reward_integral: 0, reward_remaining: 0})
            );
    ...
    function addTokenReward(address _token) public onlyOwner {
        ...
        if (registeredRewards[_token] == 0) {
            //add new token to list
            rewards.push(
                RewardType({reward_token: _token, reward_pool: address(0), reward_integral: 0, reward_remaining: 0})
            );

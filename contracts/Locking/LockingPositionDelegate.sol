 ...
 */
contract LockingPositionDelegate is Ownable2StepUpgradeable {
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        require(msg.sender == lockingPositionManager.ownerOf(tokenId), "TOKEN_NOT_OWNED");
        _;
    }
    ...
    /**
     * @notice Set the Locking Position Manager.
     * @param _lockingPositionManager ERC721 Locking Position Manager contract
     */
    function setLockingPositionManager(ILockingPositionManager _lockingPositionManager) external onlyOwner {
    ...
    /**
     * @notice  Obtain the delegatees for a token (NFT).
    ...
    /**
     * @notice Delegates a percentage of mgCVG for a tokenId to other addresses.
     * @param _tokenId is the ID of the token (NFT) to delegate voting power.
     * @param _delegateesData data of the delegatees (address and percentage).
     */
    function delegateMgCvg(uint256 _tokenId, MgCvgDelegatee[] calldata _delegateesData) external onlyTokenOwner(_tokenId) {
        for (uint256 i; i < _delegateesData.length;) {
            _delegateMgCvg(_tokenId, _delegateesData[i].delegatee, _delegateesData[i].percentage);
            unchecked { ++i; }
        }
    }
     ...
     * @dev A percentage of 0 can be used to remove a delegation.
     * @param _tokenId is the ID of the token (NFT) to delegate voting power.
     * @param _to is the address we want to delegate to.
     * @param _percentage is the percentage we want to delegate to the address.
     */
    function _delegateMgCvg(uint256 _tokenId, address _to, uint96 _percentage) internal {
        ...
        /// @dev we should ensure it's an update of a delegatee or maximum delegation hasn't been reached
        uint256 _delegateesLength = delegatedMgCvg[_tokenId].length;
        require(_delegateesLength < maxMgDelegatees || _isUpdate, "TOO_MUCH_DELEGATEES");

        /// @dev target delegatee cannot have more tokens delegated than maximum
        uint256 tokenIdsDelegated = mgCvgDelegatees[_to].length;
        require(tokenIdsDelegated < maxTokenIdsDelegated || _isUpdate, "TOO_MUCH_MG_TOKEN_ID_DELEGATED");

        /// @dev the total percentage delegated for this token mustn't be greater than 100%
        ...
        /// @dev it must either be an update of a delegatee or percentage must be greater than 0
     ...
     * @param _ownedAndDelegatedTokens array of owned/veDelegated/mgDelegated tokenIds allowed, arrays must be sorted in ascending order
     */
    function manageOwnedAndDelegated(OwnedAndDelegated calldata _ownedAndDelegatedTokens) external {
        ...
        for (uint256 i; i < _ownedAndDelegatedTokens.owneds.length; ) {
            /// @dev ensure uniqueness to prevent duplicated values
            if (i != 0)
                require(
                    _ownedAndDelegatedTokens.owneds[i - 1] < _ownedAndDelegatedTokens.owneds[i],
                    "INVALID_OWNED_VALUE"
                );
        ...
        /// @dev Check Ownership on all owned tokens
        lockingPositionManager.checkOwnerships(_ownedAndDelegatedTokens.owneds, msg.sender);
        ...
        for (uint256 i; i < _ownedAndDelegatedTokens.mgDelegateds.length; ) {
            ...
            if (i != 0)
                require(
                    _ownedAndDelegatedTokens.mgDelegateds[i - 1] < _ownedAndDelegatedTokens.mgDelegateds[i],
                    "INVALID_MG_VALUE"
                );
        ...
        for (uint256 i; i < _ownedAndDelegatedTokens.veDelegateds.length; ) {
            ...
            if (i != 0)
                require(
                    _ownedAndDelegatedTokens.veDelegateds[i - 1] < _ownedAndDelegatedTokens.veDelegateds[i],
                    "INVALID_VE_VALUE"
                );
    ...
    /**
     * @notice Method to clean all associated delegatees on token transfer.
     * @param _tokenId to remove
     */
    function cleanDelegateesOnTransfer(uint256 _tokenId) external {
        require(address(cvgControlTower.lockingPositionManager()) == msg.sender, "NOT_LOCKING_MANAGER");

        _cleanVeDelegatee(_tokenId, false);
        _cleanMgDelegatees(_tokenId);

        delegatedYsCvg[_tokenId] = address(0);
    }

...
import abiRouter from "../../abis/UniswapRouterV3.json";

const routerUniV3 = "0xE592427A0AEce92De3Edee1F18E0157C05861564";

export async function manipulateUniV3Lp(lpAddress: string, actions: any[], user: Signer) {
    ...
    await (await tokens[0].connect(user).approve(router3Uniswap, ethers.MaxUint256)).wait();
    await (await tokens[1].connect(user).approve(router3Uniswap, ethers.MaxUint256)).wait();
        ...
        switch (actionType) {
            case "swap":
                const inputParams = {
                    ...
                    amountIn: ethers.parseEther(action.amountIn.toString()),
                    amountOutMinimum: 0,
                    sqrtPriceLimitX96: 0,
                };

...
export class ApiHelper {
    ...
    static async getDefiLlamaTokenPrices(tokensAddresses: string[], searchWidth = "4h"): Promise<any> {
        ...
        } else if (network.config.forking.blockNumber) {
            timestamp = (await ethers.provider.getBlock(network.config.forking.blockNumber))!.timestamp;
        }
        priceUri = `${ApiHelper.defiLlamaApiEndpoint}/prices/historical/${timestamp.toString().toString()}/ethereum:${tokensCall}?searchWidth=${searchWidth}`;
    ...
    static async verifyContractEthernal(name: string, address: string, abi: any) {
        ...
        try {
            ...
            console.log(`ERROR  on ${name} verification !`);
        }
    }
}
